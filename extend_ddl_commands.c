...
static void ExtendDDLCommandNames(Node *parseTree, uint64 shardId);
...
ExtendedDDLCommandList(Oid masterRelationId, uint64 shardId, List *ddlCommandList)
{
	...
	foreach(ddlCommandCell, ddlCommandList)
	{
		...
		ExtendDDLCommandNames(ddlCommandNode, shardId);
...
/*
 * ParseTreeNode parses the given DDL command, and returns the tree node for the parsed command.
 */
static Node *
...
/*
 * ExtendDDLCommandNames extends relation names in the given parse tree for
 ...
 */
static void
ExtendDDLCommandNames(Node *parseTree, uint64 shardId)
{
...
DeparseCreateStmt(CreateStmt *createStmt, Oid masterRelationId)
{
	...
	RangeTblEntry *rte = addRangeTableEntryForRelation(parseState, masterRelation,
													   NULL, false, true);
	...
	*/
	...
	foreach(tableElementCell, tableElementList)
	{
		...
		checkContext = deparse_context_for(masterRelationName,
										   masterRelationId);
 		checkString = deparse_expression(plannedExpression, checkContext,
										 false, false);
 ...
 * in the expression.
 * Note: This function is pulled as-is from src/backed/catalog/heap.c.
 */
static Node *
...
DeparseIndexStmt(IndexStmt *indexStmt, Oid masterRelationId)
{
	...
	*/
	...
	if (indexStmt->unique)
	{
		appendStringInfoString(buffer, "CREATE UNIQUE INDEX");
	}
	else
	{
		appendStringInfoString(buffer, "CREATE INDEX");
	}

	Assert(indexStmt->idxname != NULL);
	appendStringInfo(buffer, " %s ON %s USING %s (",
	...
	if (indexStmt->options)
	{
		...
		foreach(optionCell, optionList)
		{
			...
			appendStringInfo(buffer, "%s=", quote_identifier(optionName));
			appendStringInfo(buffer, "%s", optionValue);
		}
